...
export function PremiumHero({
  ...
  return (
    <section className="premium-hero-surface relative isolate overflow-hidden border-b border-slate-900 bg-slate-950 text-white">
      ...
      <div className="bureau-container grid gap-8 py-16 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-end lg:py-20">
        <div className="space-y-6">
          ...
          <h1 className="max-w-5xl text-4xl font-semibold leading-[1.05] tracking-normal text-balance sm:text-5xl lg:text-6xl">
            {title}
          </h1>
        ...
        {aside ? (
          <div className="premium-card-glow rounded-md border border-white/10 bg-white/[0.065] p-5 shadow-2xl shadow-slate-950/35 backdrop-blur">
            {aside}
          </div>
        ) : null}
      </div>
...
export function PremiumButton({
  ...
  return (
    <Button
      ...
      className={
        variant === "primary"
          ? "bureau-hover-lift bg-amber-500 text-slate-950 shadow-lg shadow-amber-950/20 hover:bg-amber-400"
          : "bureau-hover-lift border-white/20 bg-white/10 text-white hover:bg-white/15"
      }
    >
      <Link href={cta.href}>
        {cta.label}
...
export function PremiumSectionHeader({
  ...
  return (
    <div className="max-w-3xl space-y-3">
      ...
      <h2 className={dark ? "text-3xl font-semibold tracking-normal text-balance text-white sm:text-4xl" : "text-3xl font-semibold tracking-normal text-balance text-slate-950 sm:text-4xl"}>
        {title}
      </h2>
...
export function PremiumFeatureCard({
  ...
  return (
    <div className={dark ? "premium-card-glow rounded-md border border-white/10 bg-white/[0.06] p-5" : "bureau-hover-lift rounded-md border border-slate-200 bg-white p-5 shadow-sm"}>
  ...
  return (
    <div className={dark ? "border-y border-white/10 bg-slate-950 text-white" : "border-y border-slate-200 bg-white"}>
      <div className="bureau-container grid gap-4 py-6 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((item) => (
          <div key={item.label} className={dark ? "rounded-md border-l border-amber-300/70 bg-white/[0.035] p-4" : "rounded-md border-l border-amber-300 bg-slate-50 p-4"}>
    ...
    <section className="premium-hero-surface bureau-section relative isolate overflow-hidden bg-slate-950 text-white">
      ...
      <div className="bureau-container grid gap-8 lg:grid-cols-[1fr_0.72fr] lg:items-center">
        <div className="space-y-4">
          ...
          <h2 className="text-4xl font-semibold tracking-normal text-balance sm:text-5xl">{title}</h2>
    ...
    <BureauPanel dark={dark}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className={dark ? "mt-3 text-3xl font-semibold text-white" : "mt-3 text-3xl font-semibold text-slate-950"}>
            {value}
          </p>
        </div>
        {Icon ? (
    ...
    <BureauPanel className="overflow-hidden p-0">
      <div className="bg-slate-950 p-5 text-white">
        ...
        <div className="mt-4 flex items-end justify-between gap-4">
          <p className="text-6xl font-semibold leading-none">{clampedScore}</p>
      ...
      {facts?.length ? (
        <div className="grid gap-0 divide-y divide-slate-100">
          {facts.map((fact) => (
            <div key={fact.label} className="flex items-center justify-between gap-4 px-5 py-3 text-sm">
      ...
      {items.map((item, index) => {
        ...
        return (
          <div
            ...
            className={cn(
              "grid gap-4 border-b p-5 last:border-b-0 lg:grid-cols-[86px_54px_1fr_auto] lg:items-center",
              dark ? "border-white/10" : "border-slate-200",
            )}
          >
            <span className={dark ? "text-2xl font-semibold text-white" : "text-2xl font-semibold text-slate-950"}>{phase}</span>
            <span className={dark ? "flex size-12 items-center justify-center rounded-md bg-amber-300 text-slate-950" : "flex size-12 items-center justify-center rounded-md bg-slate-950 text-white"}>
              <Icon className="size-5" aria-hidden="true" />
            </span>
            <div>
              <h3 className={dark ? "text-xl font-semibold text-white" : "text-xl font-semibold text-slate-950"}>{item.title}</h3>
            ...
            {item.href && item.cta ? (
              <Button asChild variant="outline" className={dark ? "border-white/20 bg-white/10 text-white hover:bg-white/15 lg:justify-self-end" : "lg:justify-self-end"}>
                <Link href={item.href}>
                  {item.cta}
